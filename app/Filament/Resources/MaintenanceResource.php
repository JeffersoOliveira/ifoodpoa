        ...
        return $table
            ...
            ->actions([
                ...
                Action::make('Manutençao')
                    // ->url(function ($record) {
                    //     route('filament.admin.resources.maintenance.repairrecords', ['record' => $record->id]);
                    //     // dd($record);
                    // })
                    ...
                    ->visible(function ($record) {
                        ...
                        // dd($record->repair?->mechanic_id === auth()->id() and $record->status != MaintenanceStatusEnum::COMPLETED);
                        if ($record->repair?->mechanic_id === auth()->id() and $record->status != MaintenanceStatusEnum::COMPLETED) {
                            return true;
                        }
                        // dd($record->repair->mechanic_id === auth()->id());
                        return !$record->repair()->exists();
                        //                        return auth()->user()->role->name;
//                        dd(auth()->user()->role->name === 'mechanics');
                        // if (auth()->user()->role->name === 'mechanics') {
                        //     return true;
                        // }
                        // return true;
